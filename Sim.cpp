...
#define MAX_SLOTS 352	//Total number of frequency slots 352 (88 channels * 4 slots)
...
#define MAX_TRIAL_PATHS 4		//Changes when graph changes (Total posible paths for primary and backup)
//=======================CHANGES WHEN GRAPH CHANGES==============================================

//=========================SIMULATION SETUP======================================================
#define MAX_LIGHTPATHS 100	//Total number of Lightpaths
#define MeanHoldTime 1		//IHT is Inter Holding Time (Mean). Typically it is 1.
#define DEF_LOAD 100	 //Load. and it varies.
//=========================SIMULATION SETUP======================================================

#define SHLPIDSTART 1001		//Initial Shared ID of a shared_id map (Start from +MAX_LIGHTPATHS)

//=======================CHANGES WHEN LAYERS CHANGE==============================================
#define MAX_LAYERS 50		//Maximum size of layers
...
void select_slots_for_primarypath();
...
//----------------------------------FOR GRAPH STARTS---------------------------------------------
VecMap graphtextmap;			//For inserting path and it's Net_Top from Net_Top text file
...
int main(int argc, const char* argv[])
{
	...
	for(int i=1; i<=MAX_LIGHTPATHS; i++)
	{
		...
		*/
		...
		end= random_destination(min_nodes,max_nodes,start);
		cout<<"\nDestination Network Node: "<<end;
		//------------------For Destination Node-----------------------------
...
//++++++++++++++++++++++++++++++++--------AUTOMATION----------+++++++++++++++++++++++++++++++++++
		...
		if(TIME_SCALE.empty())
		{
			admit_lightpath(start, end, Lightpath);
		...
		else if(!TIME_SCALE.empty())
		{
			...
			else if(curTime>TIME_SCALE.begin()->first)
			{
				...
				{					
					...
					TIME_SCALE.erase(TIME_SCALE.begin());
...
//Net_Top
	ifstream file1 ("Net_Top.txt");
	if (! file1.is_open())
	{ cout << "Error opening file Net_Top.txt"; return; }
	...
	while(! file1.eof() )
	{
		...
		{		
			...
			graphtextmap[fiber_id].push_back(destin);			//Inserting fiberid and source-destination into graphtextmap map from Net_Top text file
			
			fiberid_weight_map.insert(make_pair(fiber_id, weight));		//Inserting fiberid and weight into fiberid_weight_map map from Net_Top text file
...
//-----------------------------SELECTING FIRST SHORTEST PATH AS PRIMARY--------------------------
void select_primary_path()
{
	ppnodes=pathsmmap.begin()->second;
...
void del_back_lp_from_mainmap (int dLightpath)
{
	...
	{
		cout<<"\n\nNO SUCH LIGHTPATH EXIST TO DELETE!!!!\n";
	}
...
void admit_lightpath(int start, int end, int Lightpath)
{
	...
	//===============================================================================================
	select_primary_path();			//Selecting first shortest path as primary path
	...
	else if(pathsmmap.size()>=1)
	{
		...
		if (lpchecker(prislotposition))
		{
			...
			for(VecMul::iterator pmmap=pathsmmap.begin(); pmmap != pathsmmap.end(); ++pmmap)
			{
				bpnodes=pmmap->second;
			...
			//-----For only "no slots for backup path"
			...
			//-----For only "no additional condition satisfied"
...
map<int, int> BothNullLinkOperation (int node)
{
	...
	if (itmain != mainnodemap.end())
	{
		...
		for(size_t j=0; j<mainVec.size(); j++)
		{
			if(mainVec[j]==0)
			{
				NodeLayer.insert(make_pair(node,j+1));			//(i+1) - START LAYER FROM 1
				break;
			}					
		}				
	}
			...
			for(IntVec::iterator itd=delnodes.begin(); itd!=delnodes.end();++itd)
			{
				for(map<int, set<int> >::iterator it=node_lps.begin(); it!=node_lps.end(); )
				{
					...
					if((*itd)==key)
					{
						...
						itVal.erase(remove(itVal.begin(), itVal.end(), (dLightpath)), itVal.end());
	...
	{		
		for(MapSet::iterator itk = bplink_lps.begin(); itk != bplink_lps.end(); )
		{
			...
			{	
				...
				dellps.erase(remove(dellps.begin(), dellps.end(), dLightpath), dellps.end());  // remove 'value' from vector
...
void del_lightpath_from_splitmulmap(int dLightpath)
{
	for(TriMul::iterator it = SplitMulMap.begin(); it != SplitMulMap.end();++it)
	{
		...
		for(MapSet::iterator itm = Sec.begin(); itm != Sec.end(); )
		{
			...
			{			
				SetVal.erase(remove(SetVal.begin(), SetVal.end(), dLightpath), SetVal.end());			
...
void Calculation()
{
	...
	ofstream fout ("PBPS_in_EON_Results.txt",ios::out | ios::app);
